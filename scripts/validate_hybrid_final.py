"""
Гибридный бэктест:
LONG Asian/London: simple windows (ATR=14, без range фильтров)
LONG NY: с range фильтрами из e77adbe (ATR=20)
SHORT: без изменений (ATR=14)
"""
import sys
import os
...
print("="*80)
print("ГИБРИД: Asian/London simple + NY range фильтры + SHORT")
print("="*80)
...
# Asian/London: Simple windows (ATR=14)
ASIAN_SIMPLE = {
    'breakout_hours': (7, 10),
    'atr_period': 14
}
...
for date in unique_dates:
    ...
    # Session tracking for simple windows
    ...
    for i in range(len(day_data)):
        ...
        if active_long is None:
            # Track session highs/lows for ASIAN and LONDON
            if ASIAN_SIMPLE['breakout_hours'][0] <= hour < ASIAN_SIMPLE['breakout_hours'][1]:
                if asian_session_high is None:
                    asian_session_high = highs[i]
                    asian_session_low = lows[i]
                else:
                    asian_session_high = max(asian_session_high, highs[i])
                    asian_session_low = min(asian_session_low, lows[i])

            if LONDON_SIMPLE['breakout_hours'][0] <= hour < LONDON_SIMPLE['breakout_hours'][1]:
                if london_session_high is None:
                    london_session_high = highs[i]
                    london_session_low = lows[i]
                else:
                    london_session_high = max(london_session_high, highs[i])
                    london_session_low = min(london_session_low, lows[i])

            # Check for ASIAN breakout
            if asian_session_high is not None and hour >= ASIAN_SIMPLE['breakout_hours'][0]:
                if closes[i] > asian_session_high:
                    if USE_H4_EMA_FILTER:
                        if pd.isna(current_h4['ema20']) or current_h4['close'] < current_h4['ema20']:
                            pass
                        else:
                            entry = closes[i]
                            sl = asian_session_low - ATR_BUFFER * atr_14
                            risk = entry - sl
            ...
            if active_long is None and london_session_high is not None and hour >= LONDON_SIMPLE['breakout_hours'][0]:
                if closes[i] > london_session_high:
                    if USE_H4_EMA_FILTER:
                        if pd.isna(current_h4['ema20']) or current_h4['close'] < current_h4['ema20']:
                            pass
                        else:
                            entry = closes[i]
                            sl = london_session_low - ATR_BUFFER * atr_14
                            risk = entry - sl
            ...
            # NY with range filters
            if active_long is None and NY_PARAMS['breakout_hours'][0] <= hour < NY_PARAMS['breakout_hours'][1]:
                range_start, range_end = NY_PARAMS['range_hours']
                ...
                if len(range_data) > 0:
                    ...
                    min_range = NY_PARAMS['min_range_atr'] * atr_20
                    max_range = NY_PARAMS['max_range_atr'] * atr_20
                    ...
                    if min_range <= range_size <= max_range:
                        if closes[i] > range_high:
                            ...
                            entry = closes[i]
                            sl = range_low - NY_PARAMS['stop_buffer_atr'] * atr_20
                            risk = entry - sl
...
for session in ['asian', 'london', 'ny']:
    ...
    if len(session_trades) > 0:
        ...
        print(f"{session.upper()}: 0 trades")
print()
print("="*80)
