"""
TEST 2: SHORT до 17 UTC БЕЗ NY LONG
====================================
Контрольный тест: ограничить SHORT до 17 UTC, но NY LONG не добавлять

Изменения от v3.0:
1. SHORT: входить только если hour < 17 (вместо < 21)
2. LONG: только Asian (7-10) + London (13-16), БЕЗ NY

Цель: Понять сколько SHORT трейдов теряем при ограничении до 17 UTC
"""
import sys
import os
...
print("="*80)
print("TEST 2: SHORT до 17 UTC БЕЗ NY LONG")
print("="*80)
print(f"Period: {df.index[0]} - {df.index[-1]}")
print()
print("ИЗМЕНЕНИЯ:")
print("  - SHORT: входить только если hour < 17 (вместо < 21)")
print("  - LONG: только Asian + London (БЕЗ NY)")
print()
...
print("="*80)
print(f"TEST 2:   {len(trades_df)} trades, ${trades_df['pnl'].sum():,.0f} PnL, DD {max_dd:.2f}%")
print(f"v3.0:     557 trades, $69,520 PnL, DD 6.65%")
print(f"Разница:  {len(trades_df) - 557:+d} trades, ${trades_df['pnl'].sum() - 69520:+,.0f} PnL")
print()
print(f"SHORT trades lost (17-21 UTC): {188 - len(short_df)} trades")
print()
