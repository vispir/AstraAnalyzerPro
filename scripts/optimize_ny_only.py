"""
Per-Session Optimization for Session Range Breakout
Optimizes each session (Asian, London, NY) separately with independent parameters
"""
import sys
import os
import json
...
GLOBAL_DF = None
GLOBAL_SESSION = None  # Which session to optimize: 'asian', 'london', 'ny'
...
TRAILING_START_VALUES = [None, 1.5, 2.0]
...
def run_single_combination(args):
    ...
    try:
        ...
        # Filter: DD < 10%, daily DD < 5%, trades >= 50 (NY has fewer trades)
        passes_filter = (
...
def run_optimization():
    print("=== Per-Session Optimization for Session Range Breakout ===")
    ...
    # Optimize only NY session
    all_results = {}

    for session_name in ['ny']:
        print(f"\n{'='*80}")
        ...
        output_file = f"backtest_results/{session_name}_session_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    for session_name, results in all_results.items():
        best = max(results, key=lambda x: x['summary']['total_pnl'] if x.get('passes_filter', False) else -999999)
        if best.get('passes_filter', False):
            p = best['params']
            s = best['summary']
            trail_str = f"{p['trailing_start']:.1f}R" if p['trailing_start'] is not None else "None"
            print(f"\n{session_name.upper()}: PnL=${s['total_pnl']:,.0f}, PF={s['profit_factor']:.3f}, DD={s['max_drawdown_pct']:.2f}%")
            print(f"  Params: TP_RR={p['tp_rr']}, STOP={p['stop_buffer_atr']}, MIN_R={p['min_range_atr']}, MAX_R={p['max_range_atr']}, Trail={trail_str}")
