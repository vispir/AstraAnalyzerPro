"""
Валидация оригинальных файлов из коммита 865b8e1
session_breakout_trader.py (LONG) + short_reversal_v1.py (SHORT)
"""
import sys
import os
...
print("="*80)
print("ВАЛИДАЦИЯ КОММИТА 865b8e1")
print("="*80)
print(f"Period: {df.index[0]} - {df.index[-1]}")
print()

# Parameters from commit 865b8e1
RISK_PER_TRADE = 158
TP_RR = 5.5
ATR_PERIOD = 14
ATR_BUFFER = 0.5
...
# LONG: Simple session windows (no range filters)
LONG_SESSIONS = {
    'asian': (7, 10),
    'london': (13, 16),
    'ny': (18, 21)
...
for date in unique_dates:
    ...
    for i in range(len(day_data)):
        ...
        # LONG LOGIC (simple session windows)
...
print("="*80)
print("СРАВНЕНИЕ С КОММИТОМ 865b8e1")
print("="*80)
print(f"Ожидаемо: 606 trades, $57,274 PnL")
print(f"Получено: {len(trades_df)} trades, ${trades_df['pnl'].sum():,.0f} PnL")
print()
