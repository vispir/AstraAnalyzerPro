"""
Drawdown Analysis for LONG + SHORT Combined Strategy
"""
import pandas as pd
import numpy as np
...
print("="*80)
print("DRAWDOWN ANALYSIS: LONG + SHORT (2020-2026)")
print("="*80)
...
print(f"Total trades: {len(trades_df)} (LONG: {len(long_trades)}, SHORT: {len(short_trades)})")
print()
...
print("="*80)
if abs(max_dd) > 10:
    print(f"⚠️  WARNING: Max DD {max_dd:.2f}% EXCEEDS 10% limit")
else:
    print(f"✓ Max DD {max_dd:.2f}% is within 10% limit")

if abs(worst_day_pct) > 5:
    print(f"⚠️  WARNING: Worst day {worst_day_pct:.2f}% EXCEEDS 5% limit")
else:
    print(f"✓ Worst day {worst_day_pct:.2f}% is within 5% limit")
print("="*80)
