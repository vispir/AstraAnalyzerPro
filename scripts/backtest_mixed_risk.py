"""
Combined Strategy Backtest: LONG Risk=$200, SHORT Risk=$170
"""
import sys
import os
...
# Parameters
LONG_RISK = 200  # CHANGED FROM 158
SHORT_RISK = 170  # CHANGED FROM 158
TP_RR = 5.5
ATR_PERIOD = 14
ATR_BUFFER = 0.5
H4_EMA_PERIOD = 20
...
print("="*80)
print(f"BACKTEST: LONG Risk=${LONG_RISK}, SHORT Risk=${SHORT_RISK}")
print("="*80)
    ...
    for i in range(len(day_data)):
        ...
        if active_long is not None:
            ...
            if lows[i] <= active_long['sl']:
                pnl = (active_long['sl'] - active_long['entry']) / (active_long['entry'] - active_long['initial_sl']) * LONG_RISK
                balance += pnl
                trades.append({'date': active_long['entry_time'], 'exit_date': times[i], 'pnl': pnl, 'direction': 'LONG'})
                active_long = None
            elif highs[i] >= active_long['tp']:
                pnl = (active_long['tp'] - active_long['entry']) / (active_long['entry'] - active_long['initial_sl']) * LONG_RISK
                balance += pnl
                trades.append({'date': active_long['entry_time'], 'exit_date': times[i], 'pnl': pnl, 'direction': 'LONG'})
                active_long = None
        ...
        if active_short is not None:
            ...
            if highs[i] >= active_short['sl']:
                pnl = (active_short['entry'] - active_short['sl']) / (active_short['initial_sl'] - active_short['entry']) * SHORT_RISK
                balance += pnl
                ...
                pnl = (active_short['entry'] - active_short['tp']) / (active_short['initial_sl'] - active_short['entry']) * SHORT_RISK
                balance += pnl
                trades.append({'date': active_short['entry_time'], 'exit_date': times[i], 'pnl': pnl, 'direction': 'SHORT'})
                active_short = None
                short_type1_reversal_active = False
...
for year in sorted(trades_df['year'].unique()):
    ...
    yearly_stats.append({
        ...
        'long_trades': len(year_long),
        'short_trades': len(year_short),
        'pnl': year_trades['pnl'].sum(),
        'long_pnl': year_long['pnl'].sum(),
        'short_pnl': year_short['pnl'].sum(),
        'win_rate': len(year_trades[year_trades['pnl'] > 0]) / len(year_trades) * 100
    })

print("="*80)
print(f"RESULTS: LONG Risk=${LONG_RISK}, SHORT Risk=${SHORT_RISK}")
print("="*80)
print()
...
print("="*80)
print(f"{'Year':<6} | {'Trades':<7} | {'LONG':<5} | {'SHORT':<5} | {'Total PnL':<12} | {'LONG PnL':<10} | {'SHORT PnL':<10} | {'WR':<6}")
print("-"*80)
for stat in yearly_stats:
    print(f"{stat['year']:<6} | {stat['trades']:<7} | {stat['long_trades']:<5} | {stat['short_trades']:<5} | ${stat['pnl']:>10,.0f} | ${stat['long_pnl']:>8,.0f} | ${stat['short_pnl']:>9,.0f} | {stat['win_rate']:>5.1f}%")

print()
print("="*80)
print("COMPARISON WITH BASELINE (Risk=$158 for both)")
print("="*80)
print()

# Baseline results
baseline_total_pnl = 55932
baseline_long_pnl = 37200
baseline_short_pnl = 18732
baseline_dd = 6.84

long_ratio = LONG_RISK / 158
short_ratio = SHORT_RISK / 158

print(f"Risk Ratios:")
print(f"  LONG: ${LONG_RISK} / $158 = {long_ratio:.3f}x")
print(f"  SHORT: ${SHORT_RISK} / $158 = {short_ratio:.3f}x")
print()

print(f"Total PnL:")
print(f"  Current: ${trades_df['pnl'].sum():,.0f}")
print(f"  Baseline: ${baseline_total_pnl:,.0f}")
print(f"  Difference: ${trades_df['pnl'].sum() - baseline_total_pnl:+,.0f} ({(trades_df['pnl'].sum() / baseline_total_pnl - 1) * 100:+.1f}%)")
print()

print(f"LONG PnL:")
print(f"  Current: ${long_df['pnl'].sum():,.0f}")
print(f"  Baseline: ${baseline_long_pnl:,.0f}")
print(f"  Expected: ${baseline_long_pnl * long_ratio:,.0f}")
print(f"  Difference: ${long_df['pnl'].sum() - baseline_long_pnl:+,.0f} ({(long_df['pnl'].sum() / baseline_long_pnl - 1) * 100:+.1f}%)")
print()

print(f"SHORT PnL:")
print(f"  Current: ${short_df['pnl'].sum():,.0f}")
print(f"  Baseline: ${baseline_short_pnl:,.0f}")
print(f"  Expected: ${baseline_short_pnl * short_ratio:,.0f}")
print(f"  Difference: ${short_df['pnl'].sum() - baseline_short_pnl:+,.0f} ({(short_df['pnl'].sum() / baseline_short_pnl - 1) * 100:+.1f}%)")
print()

print(f"Max DD:")
print(f"  Current: {max_dd:.2f}%")
print(f"  Baseline: {baseline_dd:.2f}%")
print(f"  Change: {max_dd - baseline_dd:+.2f}%")
print()

print(f"Return:")
print(f"  Current: {(balance - 10000) / 10000 * 100:.1f}%")
print(f"  Baseline: {(10000 + baseline_total_pnl - 10000) / 10000 * 100:.1f}%")
print()
print("="*80)
