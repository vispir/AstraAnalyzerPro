"""
Combined LONG + SHORT Strategy Backtest with Validation
LONG: Session Breakout
SHORT: Type 1 (Historical High) + Type 2 (Local Reversal)
"""
import sys
import os
...
def run_combined_backtest(df, df_h4, validate_trades=False):
    trades = []
    active_long = None
    active_short = None
    balance = 10000
...
def main():
    print("=== COMBINED LONG + SHORT BACKTEST (FINAL) ===")
    print(f"Period: {START_DATE} - {END_DATE}")
    print(f"Risk per trade: ${RISK_PER_TRADE}, TP: {TP_RR}R")
    print(f"LONG: Session Breakout (H4 close > EMA20)")
    print(f"SHORT: Type 1 + Type 2 (H4 close < EMA20)\n")
    ...
    print("\n" + "="*120)
    print("TRADE VALIDATION: 5 EXAMPLES OF EACH TYPE")
    print("="*120)

    # LONG examples
    long_trades = trades_df[trades_df['direction'] == 'LONG']
    print("\nLONG TRADES (Session Breakout):")
    print("-" * 120)
    for idx, trade in long_trades.head(5).iterrows():
        direction_check = "UP" if trade['exit_price'] > trade['entry_price'] else "DOWN"
        print(f"Entry: {trade['entry_price']:.2f}, SL: {trade['sl']:.2f}, TP: {trade['tp']:.2f}, "
              f"Exit: {trade['exit_price']:.2f} ({trade['exit_reason']}), "
              f"PnL: ${trade['pnl']:.2f}, Direction: {direction_check}")

    # SHORT Type 1 examples
    short_type1 = trades_df[trades_df.get('type') == 'Type1_HistoricalHigh']
    print("\nSHORT TYPE 1 TRADES (Historical High Reversal):")
    print("-" * 120)
    for idx, trade in short_type1.head(5).iterrows():
        direction_check = "DOWN" if trade['exit_price'] < trade['entry_price'] else "UP"
        print(f"Entry: {trade['entry_price']:.2f}, SL: {trade['sl']:.2f}, TP: {trade['tp']:.2f}, "
              f"Exit: {trade['exit_price']:.2f} ({trade['exit_reason']}), "
              f"PnL: ${trade['pnl']:.2f}, Direction: {direction_check}")

    # SHORT Type 2 examples
    short_type2 = trades_df[trades_df.get('type') == 'Type2_LocalReversal']
    print("\nSHORT TYPE 2 TRADES (Local Reversal):")
    print("-" * 120)
    for idx, trade in short_type2.head(5).iterrows():
        direction_check = "DOWN" if trade['exit_price'] < trade['entry_price'] else "UP"
        print(f"Entry: {trade['entry_price']:.2f}, SL: {trade['sl']:.2f}, TP: {trade['tp']:.2f}, "
              f"Exit: {trade['exit_price']:.2f} ({trade['exit_reason']}), "
              f"PnL: ${trade['pnl']:.2f}, Direction: {direction_check}")
