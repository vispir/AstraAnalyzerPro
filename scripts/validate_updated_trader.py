"""
Валидация обновлённого session_breakout_trader.py с range фильтрами
Ожидаемый результат: NY должен быть прибыльным, LONG ~360-389 сделок
"""
import sys
import os
...
print("="*80)
print("ВАЛИДАЦИЯ ОБНОВЛЁННОГО session_breakout_trader.py")
print("="*80)
print(f"Period: {df.index[0]} - {df.index[-1]}")
print()

# Parameters from updated session_breakout_trader.py
RISK_PER_TRADE = 158
TP_RR = 5.5
ATR_PERIOD = 20  # For LONG
SHORT_ATR_PERIOD = 14  # For SHORT
ATR_BUFFER = 0.5
USE_H4_EMA_FILTER = True
H4_EMA_PERIOD = 20

# LONG: Session parameters with range filters
ASIAN_PARAMS = {
    'tp_rr': 5.5,
    'stop_buffer_atr': 0.1,
    'min_range_atr': 0.7,
...
df['atr'] = calculate_atr(df, ATR_PERIOD)  # For LONG
df['atr_short'] = calculate_atr(df, SHORT_ATR_PERIOD)  # For SHORT
...
for date in unique_dates:
    ...
    for i in range(len(day_data)):
        ...
        if np.isnan(atr) or np.isnan(atr_short):
            continue
...
print("VALIDATION")
print("="*80)
print(f"NY Session: {'PROFITABLE' if long_df[long_df['session'] == 'ny']['pnl'].sum() > 0 else 'UNPROFITABLE'}")
print(f"LONG Trades: {len(long_df)} (expected ~360-389)")
print(f"Total DD: {max_dd:.2f}% (should be < 10%)")
print()

if long_df[long_df['session'] == 'ny']['pnl'].sum() > 0 and 350 <= len(long_df) <= 410 and max_dd < 10:
    print("="*80)
    print(">>> ВАЛИДАЦИЯ УСПЕШНА: NY ПРИБЫЛЬНЫЙ, ПАРАМЕТРЫ В НОРМЕ <<<")
    print("="*80)
else:
    print("="*80)
    print("[!] ВНИМАНИЕ: Требуется дополнительная проверка")
    print("="*80)
print()
print("="*80)
