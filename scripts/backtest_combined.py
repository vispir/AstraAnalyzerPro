"""
Backtest 3: COMBINED (LONG + SHORT)
Логика из combined_strategy_backtest.py
"""
import sys
import os
...
print("="*80)
print("BACKTEST 3: COMBINED (LONG + SHORT)")
print("="*80)
...
print("="*80)
print("RESULTS: COMBINED (LONG + SHORT)")
print("="*80)
print()
...
for year in sorted(trades_df['year'].unique()):
    ...
    print(f"{year}: {len(year_trades)} trades (LONG: {len(year_long)}, SHORT: {len(year_short)}), "
          f"${year_trades['pnl'].sum():,.0f} PnL, WR {len(year_wins)/len(year_trades)*100:.1f}%")
print()
print("="*80)
