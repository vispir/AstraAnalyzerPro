"""
Быстрый тест: LONG из project_session_breakout_final.md + SHORT текущий
LONG: Range фильтры из старой памяти (Asian 0-7→7-10, London 7-12→13-16, NY 13-17→18-21)
SHORT: Type1 + Type2 (текущий)
"""
import sys
import os
...
print("="*80)
print("ТЕСТ: LONG (старая память) + SHORT (текущий)")
print("="*80)

# Parameters from OLD memory (project_session_breakout_final.md)
RISK_PER_TRADE = 158
TP_RR = 5.5
ATR_PERIOD = 20
...
for date in unique_dates:
    ...
    for i in range(len(day_data)):
        ...
        # LONG LOGIC (old memory with stop_buffer)
                    ...
                    if len(range_data) > 0:
                        ...
                        if min_range <= range_size <= max_range:
                            if closes[i] > range_high:
                                ...
                                entry = closes[i]
                                sl = range_low - params['stop_buffer'] * atr
                                risk = entry - sl
            ...
            if i > 0:
                ...
                if short_type1_reversal_active and closes[i] < prev_m15_low:
                    ...
                    if risk > 0:
                        tp = entry - risk * TP_RR
                        size = RISK_PER_TRADE / risk
                        active_short = {'entry': entry, 'sl': sl, 'initial_sl': sl, 'tp': tp, 'size': size}
                        short_type1_reversal_active = False
...
print(f"\nTotal PnL: ${trades_df['pnl'].sum():,.0f}")
print(f"  LONG: ${long_df['pnl'].sum():,.0f}")
print(f"  SHORT: ${short_df['pnl'].sum():,.0f}")
print(f"\nMax DD: {max_dd:.2f}%")
print(f"Win Rate: {len(trades_df[trades_df['pnl'] > 0]) / len(trades_df) * 100:.1f}%")

print("\nBY SESSION (LONG):")
for session in ['asian', 'london', 'ny']:
    s = long_df[long_df['session'] == session]
    if len(s) > 0:
        print(f"{session.upper()}: {len(s)} trades, ${s['pnl'].sum():,.0f}")

print(f"\nСРАВНЕНИЕ:")
print(f"LONG (старая память) + SHORT (текущий): {len(trades_df)} trades, ${trades_df['pnl'].sum():,.0f}")
print(f"v3.0 (NO NY):                           557 trades, $69,520")
