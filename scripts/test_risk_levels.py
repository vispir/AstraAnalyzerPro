...
df = df_full[df_full.index < '2024-01-01']
df_h4 = df_h4_full[df_h4_full.index < '2024-01-01']

print(f"Loaded {len(df)} M15 bars for period 2020-2023")
sys.stdout.flush()
...
risk_values = [180, 185, 190]

print("\nTesting RISK values on period 2020-2023")
print("="*60)
sys.stdout.flush()
...
print("\nRESULTS")
print("Risk    PnL          PF      DD      DailyDD")
print("-"*60)
sys.stdout.flush()

for r in results:
    print(f"${r['risk']:<6} ${r['pnl']:<11,.0f} {r['pf']:<7.3f} {r['dd']:<7.2f} {r['daily_dd']:<7.2f}")
    sys.stdout.flush()

best = [r for r in results if r['dd'] < 10.0]
if len(best) > 0:
    best.sort(key=lambda x: x['risk'], reverse=True)
    b = best[0]
    print(f"\nMAX RISK with DD < 10%: ${b['risk']}")
    print(f"PnL: ${b['pnl']:,.0f}, PF: {b['pf']:.3f}, DD: {b['dd']:.2f}%, DailyDD: {b['daily_dd']:.2f}%")
else:
    print("\nNo risk level found with DD < 10%")
sys.stdout.flush()
