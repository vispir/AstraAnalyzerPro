"""
Asian Session TrailDist Optimization
Tests TrailDist=[0.2, 0.3, 0.4, 0.5] with fixed TrailStart=3.5
"""
import sys
import os
...
TRAIL_DIST_VALUES = [0.2, 0.3, 0.4, 0.5]
...
if __name__ == "__main__":
    print("=" * 100)
    print("=== ASIAN SESSION TRAILDIST OPTIMIZATION ===")
    print(f"Period: {START_DATE} to {END_DATE}")
    print(f"Testing TrailDist={TRAIL_DIST_VALUES} with fixed TrailStart=3.5")
    print("=" * 100)
    print()
    ...
    for trail_dist in TRAIL_DIST_VALUES:
        ...
        asian_params['trailing_distance'] = trail_dist
    ...
    print("=== RESULTS TABLE ===")
    print("=" * 100)
    print(f"{'TrailDist':<12} {'Total PnL':<15} {'Asian PnL':<12} {'PF':<8} {'DD%':<8} {'DailyDD%':<10} {'Status':<8}")
    print("-" * 100)

    for trail_dist, r in results:
        status = "PASS" if r['passes_filters'] else "FAIL"
        print(f"{trail_dist:<12.1f} ${r['total_pnl']:<14,.0f} ${r['asian_pnl']:<11,.0f} {r['profit_factor']:<8.3f} "
