"""
Backtest 2: ТОЛЬКО SHORT
Логика из short_reversal_v1.py
"""
import sys
import os
    ...
    for i in range(len(day_data)):
        ...
        if active_short is not None:
            ...
            if highs[i] >= active_short['sl']:
                ...
                balance += pnl
                trades.append({'date': active_short['entry_time'], 'exit_date': times[i], 'pnl': pnl})
                active_short = None
                short_type1_reversal_active = False
                short_type2_reversal_active = False
