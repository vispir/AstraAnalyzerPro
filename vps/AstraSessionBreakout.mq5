...
//--- Input parameters
input double RiskPercent = 1.58;        // Risk per trade (% of balance)
...
datetime lastCandleSyncTime = 0;
int candleSyncInterval = 900;  // 15 minutes
...
void OnTick()
{
   ...
   if(TimeCurrent() - lastCandleSyncTime >= candleSyncInterval)
   {
      SyncCandlesToFile();
      lastCandleSyncTime = TimeCurrent();
   }
...
void CheckNewSignals()
{
   ...
   {
      // No signals file yet - normal on first run
      return;
   }
   ...
   // Parse JSON (simplified - assumes array of signals)
   // In production, use proper JSON parser library

   // For now, delete file after reading to prevent re-processing
   FileDelete(SignalsFile, FILE_COMMON);

   Print("New signals detected - processing...");
   // TODO: Parse JSON and open trades
}
...
{
   int barsToSync = 300;  // Last 300 M15 bars (~3 days)
...
void UpdateTrailingStops()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
      {
         ...
         if(newSL > currentSL + _Point * 10)  // Only move if significant change
         {
...
bool OpenTrade(string direction, double entry, double sl, double tp, double riskUSD)
{
   ...
   if(OrderSend(request, result))
   {
      Print("Trade opened: ", result.order, " | ", direction, " | Lot: ", lot);
      return true;
   }
   else
   {
