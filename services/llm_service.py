...
class LLMService:
    ...
    "  * Keep SL within 1.0–2.0×ATR from entry (Gold M15 typical: $8–16)\n"
    "  * SL distance from entry < 1.0×ATR or > 2.0×ATR → REJECT (technical error)\n"
    "  * Typical SL for Gold M15: 1.2-1.8×ATR ($10-14 with ATR=$8)\n"
    ...
    "- ПРАВИЛО TP:\n"
    "  * Цель должна обеспечивать R:R >= 1.5 (идеально >= 2.0) при корректном SL.\n"
    "- Приоритет выбора TP:\n"
    ...
    "- Если ни один уровень не даёт R:R >= 1.5 — выдавай WAIT.\n"
    "- Исключение: если цена на историческом максимуме/минимуме и нет уровней выше/ниже — TP = entry ± 2.0×ATR.\n"
    ...
    "\n"
    "РАСЧЁТ TP:\n"
    "- Те же правила что для Range Breakout: ближайший уровень в направлении сделки, R:R >= 1.5\n"
    "\n"
    ...
    "- SL should be within 1.0–2.0×ATR from entry (Gold M15 typical: $8–16)\n"
    "- SL distance from entry < 1.0×ATR or > 2.0×ATR → REJECT (technical error)\n"
    "\n"
    "# WHEN TO TRADE\n"
    ...
    "❌ Late/chasing entry: if impulse has already expanded and entry is no longer at a valid zone/retest (price is already extended away from setup), return WAIT and require pullback/retest entry plan\n"
    "\n"
