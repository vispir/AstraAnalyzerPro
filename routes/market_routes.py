...
def get_candles():
    try:
        ...
        # Источник данных: по умолчанию twelvedata (так как Yahoo запаздывает)
        ...
        if not timeframe:
            ...
            if "error" in h4_raw or "error" in h1_raw or "error" in m15_raw:
                return jsonify({
                    ...
                    "source": source,
                    "details": {"H4": h4_raw.get("error"), "H1": h1_raw.get("error"), "M15": m15_raw.get("error")}
                }), 500
...
def get_current_price():
    try:
        ...
        if source == 'twelvedata':
            ...
            if "candles" in res: price = res["candles"][-1]["close"]
        else:
