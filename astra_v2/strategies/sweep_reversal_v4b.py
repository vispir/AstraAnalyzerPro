...
class SweepReversalStrategyV4B(SweepReversalStrategyV4):
    """
    Variant B — Frequency + Tight SL.

    Changes vs v4:
      - Stop buffer halved (0.16 → 0.08 ATR) → tighter SL → better dollar R:R
      - TP raised to 2.0R
      - Relaxed M1 trigger kept (lookback=3, buffer=0.005 ATR)
    """

    strategy_id = "sweep_reversal_v4b"
    ...
    def _m1_trigger(
        ...
        if prior.empty:
            return None
        reclaim = atr * config.SWEEP_REVERSAL_V4B_M1_RECLAIM_ATR
        buffer = atr * config.SWEEP_REVERSAL_V4B_M1_TRIGGER_BUFFER_ATR
        swing_lookback = min(config.SWEEP_REVERSAL_V4B_M1_TRIGGER_LOOKBACK, len(prior))
    ...
    def _build_signal(
        ...
        *,
        ...
        stop_buffer = atr * config.SWEEP_REVERSAL_V4B_STOP_BUFFER_ATR
        if direction == "BULLISH":
            stop_loss = sweep_extreme - stop_buffer
            risk = trigger_price - stop_loss
            if risk <= 0:
                return None
            take_profit = trigger_price + risk * config.SWEEP_REVERSAL_V4B_TP_RR
            partial_tp = trigger_price + risk * config.SWEEP_REVERSAL_V4B_PARTIAL_CLOSE_RR
        else:
            stop_loss = sweep_extreme + stop_buffer
        ...
        return Signal(
            ...
            setup_family="sweep_reversal_v4b",
