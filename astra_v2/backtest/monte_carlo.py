...
N_RUNS = 10_000
DD_THRESHOLD_PCT = 5.0   # report probability of breaching this (total DD)
...
def run_monte_carlo(
    ...
    dates = np.array(trade_dates) if trade_dates else None
    ...
    for i in range(n_runs):
        ...
        sampled_dates = dates[indices] if dates is not None else None

        final_bal, max_dd = _compute_equity_stats(sampled_pnl, start_balance)
        pf = _compute_profit_factor(sampled_pnl)

        # Calculate daily DD if dates available
        if sampled_dates is not None:
            max_daily_dd = _compute_max_daily_dd(sampled_pnl, sampled_dates, start_balance)
        else:
            max_daily_dd = 0.0
